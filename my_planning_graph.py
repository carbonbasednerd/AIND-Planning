...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            possibleNode = PgNode_a(action)
            if possibleNode.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(possibleNode)
                for s_node in self.s_levels[level]:
                    if s_node in possibleNode.prenodes:
                        possibleNode.parents.add(s_node)
                        s_node.children.add(possibleNode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_1 in node_a1.action.effect_add:
            if effect_1 in node_a2.action.effect_rem:
                return True
 
        for effect_2 in node_a2.action.effect_add:
            if effect_2 in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_p1 in node_a1.action.effect_add:
            if effect_p1 in node_a2.action.precond_neg:
                return True

        for effect_p1 in node_a1.action.effect_add:
            if effect_p1 in node_a2.action.precond_pos:
                return True
            
        for effect_p2 in node_a2.action.effect_add:
            if effect_p2 in node_a1.action.precond_neg:
                return True
            
        for effect_p2 in node_a2.action.effect_add:
            if effect_p2 in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        precond_nodes_a1 = node_a1.prenodes
        precond_nodes_a2 = node_a2.prenodes
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parents_1 in node_s1.parents:
            for parents_2 in node_s2.parents:
                if not parents_1.is_mutex(parents_2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        length=len(self.s_levels)
 
        #itrate through the goal:
        for goal in self.problem.goal:
            i=0
            while i < length:
                for node_s in self.s_levels[i]:
                    if goal==node_s.symbol:
